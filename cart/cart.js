...
$.getJSON('chart_data.json', function(data) {
    ...
    function ShowAllCart() {
        if ($.isEmptyObject(cart)) {
            var out = '<p class="pusto"><br><br><br><br>В ВАШЕЙ КОРЗИНЕ ЕЩЁ НИЧЕГО НЕТ :(</p><br><a href="../root/catalog.php" class="silka">ПЕРЕЙТИ В КАТАЛОГ</a>';
            $('.zakaz').html(out);
        } else {
            var out = '';
            for (var key in cart) {
                out += '<div id="product"><p>' + '<img src="' + goods[key].picture + '"width="150" height="200"></img></p>';
                out += '<p>' + goods[key].product_name + '<br>' + '<br>';
                out += '<span class="Value" >Количество: ' + '<button class="minus_goods" data-art="' + key + '">-</button>' + '&nbsp;' + cart[key] + '&nbsp;' + '<button class="plus_goods" data-art="' + key + '">+</button></span><br>';
